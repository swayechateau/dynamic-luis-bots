...
getCurrentUser:(id)=>{
    ...
    return axios.get(`${host}/api/user/${id}`).then((response)=>{
      users=response.data; return axios.get(`${host}/api/permission/`)
    }).then((response)=>{
        response.data.forEach((item,index)=>{
          if(item.userId === users._id){
            current = {id:users._id,name:users.name,perm:{admin:item.admin,wizard:item.wizard,department:item.department}}
            console.log(current)
            return current;
          }
        })
