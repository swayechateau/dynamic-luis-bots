...
getCurrentUser:(id)=>{
    var current,user;
    return axios.get(`${host}/api/user/${id}`).then((response)=>{
      user=response.data; return axios.get(`${host}/api/permission/`)
    }).then((response)=>{
        response.data.forEach((item,index)=>{
          if(item.userId === user._id){
            return current = {id:user._id,name:user.name,perm:{admin:item.admin,wizard:item.wizard,department:item.department}}
          }
        })
        }).catch((error)=>{console.log(error.response.data)});
},
postUser:(array, bool)=>{
    if(array.department !== null){array.disabled=false}else{array.disabled=true};
    console.log()
...
putUser:(id, array)=>{
    if(array.department === null){array.disabled=true}
    return axios.get(`${host}/api/permission`).then((perms)=>{
...
deleteUser:(id)=>{
    return axios.get(`${host}/api/persission`)
    .then((perms)=>{
      perms.data.forEach((item,index)=>{
        if(item.userId === id){
